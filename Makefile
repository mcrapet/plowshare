...
ETCDIR  = /etc
BINDIR  = $(PREFIX)/bin
DATADIR = ${PREFIX}/share/plowshare3
DOCDIR  = ${PREFIX}/share/doc/plowshare3
MANDIR  = ${PREFIX}/share/man/man

# Packaging
GIT_DATE:=$(shell LANG=C git log -n1 --pretty=%ci | cut -d' ' -f1)
GIT_HASH:=$(shell LANG=C git log -n1 --pretty=%h)
DISTDIR = plowshare3-snapshot-git$(subst -,,$(GIT_DATE).$(GIT_HASH))
...
install_bash_completion:
	...
	@sed -e "/cut/s,/usr/local/share/plowshare3,$(DATADIR)," etc/plowshare.completion > $(DESTDIR)$(ETCDIR)/bash_completion.d/plowshare3
